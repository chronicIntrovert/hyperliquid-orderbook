...
describe('InfoLegend', () => {
  ...
  it('when open, renders all legend sections so content is fully accessible', () => {
    ...
    expect(within(region).getByRole('heading', { name: 'Price' })).toBeInTheDocument()
    expect(within(region).getByRole('heading', { name: 'Size' })).toBeInTheDocument()
    expect(within(region).getByRole('heading', { name: 'Total' })).toBeInTheDocument()
